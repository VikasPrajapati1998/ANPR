...
def post_vehicle_data(plate_image_path, plate_number, vehicle_type):
    url = 'http://192.168.1.36:8000/vehicle_entry'  # Define your URL here
    max_attempts = 5
    attempt = 0
...
def capture_video(rtsp_url):
    ...
    while True:
        ...
        frame = process_frame(frame, None, None, None, None, None, None, None)
...
def video_feed():
    rtsp_url = request.args.get('rtsp_url')  # Get RTSP URL from query parameters
    if not rtsp_url:
        return "RTSP URL is required", 400  # Return error if RTSP URL is not provided
    
    return Response(capture_video(rtsp_url), mimetype='multipart/x-mixed-replace; boundary=frame')
...
def index():
    return '''
        ...
        <body>
            ...
            <img src="/video_feed?rtsp_url=rtsp://your_camera_url" width="640" height="480">
        </body>
        </html>
    '''
